 ...
 * ============================================================================================ */
#define KQ_MAX 16384
/* Two banks, like the script draw list (gw_script.c): scripts build into one while the overlay
 * shows the other, finished one (gw_Kit_SwapBanks when the script list flips). */
static GwKitQuad kq_bank[2][KQ_MAX];
static int nkq_bank[2];
static int kq_build;
...
const GwKitQuad *gw_Kit_ShownQuadAt(int i) {
    int b = !kq_build;
    return (i >= 0 && i < nkq_bank[b]) ? &kq_bank[b][i] : NULL;
}

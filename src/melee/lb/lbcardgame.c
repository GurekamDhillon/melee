...
#if defined(TARGET_PC)
    {
        /* comment is CARD_COMMENT_SIZE bytes and icon_data sits immediately after it. The JP
         * literal is 46 bytes in the original Shift-JIS execution charset but 65 in UTF-8, so the
         * unbounded sprintf overflows into icon_data and the next _p(icon_data)[...] dereferences
         * date digits. Format into a scratch buffer and cap the copy at the real buffer size. */
        char tmp[128];
        int n = sprintf(tmp, "%s %4d/%02d/%02d", title, time.year, time.mon + 1, time.mday);
        int k;
        if (n < 0) {
            n = 0;
        }
        if (n > (int) sizeof(_p(comment)) - 1) {
            n = (int) sizeof(_p(comment)) - 1;
        }
        for (k = 0; k < n; ++k) {
            _p(comment)[k] = tmp[k];
        }

...
static int gs_kit_record(int first, int added) {
    GwScriptDraw *d;
    if (added <= 0) return 0;
    if (gs.ndraw > 0 && gs.draw[gs.ndraw - 1].kind == GW_SDRAW_KIT &&
        gs.draw[gs.ndraw - 1].kq0 + gs.draw[gs.ndraw - 1].kqn == first) {
        gs.draw[gs.ndraw - 1].kqn += added; /* consecutive kit calls share one draw entry */
        return added;
    }
...
static int test_script_kit_mod_art(void) {
    ...
    gw_Script_Tick(); /* on_tick, on_draw: the draw list is rebuilt */
    if (gs.ndraw < 2 || gs.draw[0].kind != GW_SDRAW_TEXT || gs.draw[1].kind != GW_SDRAW_KIT) {
        gw_test_fail("kit draws are not in the draw list after the plain text (n=%d)", gs.ndraw);
        rc = 1;
        goto done;
    }
    for (k = 0; k < gs.ndraw; ++k) {
        if (gs.draw[k].kind == GW_SDRAW_KIT) n_kit += gs.draw[k].kqn;
    }
    /* icon 1 + panel 9 (fill, 4 edges, 4 corners), plus the text and rows when the kit is here */
    if (n_kit < 10 || n_kit != gw_Kit_QuadCount()) {
        gw_test_fail("kit quads: %d in the draw list, %d built", n_kit, gw_Kit_QuadCount());
        rc = 1;
    }
    {
        const GwKitQuad *icon = gw_Kit_QuadAt(0), *fill = gw_Kit_QuadAt(1);

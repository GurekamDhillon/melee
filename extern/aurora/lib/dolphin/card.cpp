...
std::array<std::filesystem::path, 2> cardBasePaths; // port patch: see CARDSetBasePath
...
void CARDSetBasePath(const char* path, const s32 chan) {
  ...
  // Port patch: g_gameName is only set by Aurora's DVD layer. A host that brings its own disc
  // code (AURORA_ENABLE_DVD=OFF) has no game name yet, get_card_full_path returns "", and the
  // path supplied here was silently dropped for <userPath>/<region>. Keep the base and resolve
  // it in CARDInit, which is given the game code.
  if (aurora::g_gameName[0] == '\0') {
    if (chan == 0 || chan == 1) {
      cardBasePaths[chan] = filePath;
    } else {
      cardBasePaths[0] = cardBasePaths[1] = filePath;
    }
    return;
  }
...
void CARDInit(const char* game, const char* maker) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (cardPaths[i].empty()) {
      cardPaths[i] = get_card_full_path(cardBasePaths[i].empty() ? cardWorkingDir : cardBasePaths[i], game,
                                        selected_card_type(), static_cast<aurora::card::ECardSlot>(i));
    }
...
s32 CARDGetStatus(const s32 chan, s32 fileNo, CARDStat* stat) {
  ...
  else if (res != aurora::card::ECardResult::NOFILE) // an empty slot is the normal answer
